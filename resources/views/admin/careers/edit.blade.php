    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.careers.update", [$career->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('opportunity_type') ? 'is-invalid' : '' }}" name="opportunity_type" id="opportunity_type" required>
                    ...
                    @foreach(App\Models\Career::OPPORTUNITY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('opportunity_type', $career->opportunity_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $career->title) }}" required>
                @if($errors->has('title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('title') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $career->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('training_mode') ? 'is-invalid' : '' }}" name="training_mode" id="training_mode" required>
                    ...
                    @foreach(App\Models\Career::TRAINING_MODE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('training_mode', $career->training_mode) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('venue') ? 'is-invalid' : '' }}" type="text" name="venue" id="venue" value="{{ old('venue', $career->venue) }}">
                @if($errors->has('venue'))
                    <div class="invalid-feedback">
                        {{ $errors->first('venue') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $career->start_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $career->end_date) }}">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.career.fields.status') }}</label>
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    <option value disabled {{ old('status', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Career::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $career->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
